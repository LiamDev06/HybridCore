...
public class MuteCommand extends ServerCommand {
    ...
    public void onConsoleCommand(CommandSender sender, String[] args) {
        if (args.length == 0) {
            sender.sendMessage("§c§lMISSING ARGUMENTS! §cSpecify a player, mute duration and a reason. Valid Usage: /mute <player> <duration> <reason>");
            return;
        }

        if (args.length == 1) {
            sender.sendMessage("§c§lMISSING ARGUMENTS! §cYou need to add a reason and a mute duration. Valid Usage: /mute <player> <duration> <reason>");
            return;
        }

        if (args.length == 2) {
            sender.sendMessage("§c§lMISSING ARGUMENTS! §cYou need to add a reason for the mute. Valid Usage: /mute <player> <duration> <reason>");
            return;
        }
        ...
        if (hybridTarget.isMuted()) {
            sender.sendMessage("§c§lALREADY MUTED! §cThis player already has an active mute on their account!");
            return;
        }

        if (hybridTarget.isBanned()) {
            sender.sendMessage("§c§lALREADY BANNED! §cThis player is currently banned from the server, therefore a mute is not necessary!");
            return;
        }

        String durationArg = args[1].toLowerCase();
        if (!durationArg.endsWith("m") && !durationArg.endsWith("h") && !durationArg.endsWith("d")) {
            sender.sendMessage("§c§lINVALID DURATION! §cThe duration suffix you entered is not valid. The allowed ones are: §6m (minutes)§c, §6h (hours)§c, §6d (days)§c!");
            return;
        }
        ...
        try {
            ...
            sender.sendMessage("§c§lINVALID DURATION! §cThe duration you entered is not valid. Please try again!");
            return;
        }
        ...
        if (timeFormatInMillis == 0L)  {
            sender.sendMessage("§c§lERROR! §cSomething went wrong with setting the mute duration. Please try again!");
            return;
        }
        ...
        sender.sendMessage("§aYou successfully muted " + hybridTarget.getRankManager().getRank().getPrefixSpace() + hybridTarget.getName() + " §afor §b" + timeFormat + " " + durationNormal + "§a, with the reason: §6" + finalReason);
    }
}
