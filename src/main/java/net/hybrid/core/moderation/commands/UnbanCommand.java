...
public class UnbanCommand extends ServerCommand {
    ...
    public void onConsoleCommand(CommandSender sender, String[] args) {
        if (args.length == 0) {
            sender.sendMessage("§c§lMISSING ARGUMENTS! §cSpecify a player to unban and add a reason, valid usage: /unban <player> <reason>");
            return;
        }

        if (args.length == 1) {
            sender.sendMessage("§c§lMISSING ARGUMENTS! §cSpecify a reason for the unban, valid usage: /unban <player> <reason>");
            return;
        }
        ...
        if (!hybridTarget.isBanned()) {
            sender.sendMessage("§c§lNOT BANNED! §cThis player is not banned and therefore cannot be unbanned!");
            return;
        }
        ...
        sender.sendMessage("§aYou successfully unbanned " + hybridTarget.getRankManager().getRank().getPrefixSpace() + hybridTarget.getName() + "§a with the reason: §6" + reason);
    }
}
