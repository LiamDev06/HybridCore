...
public class TempBanCommand extends ServerCommand {
    ...
    public void onPlayerCommand(Player player, String[] args) {
        ...
        if (finalReason.equalsIgnoreCase("cheating")) {
    ...
    public void onConsoleCommand(CommandSender sender, String[] args) {
        if (args.length == 0) {
            sender.sendMessage("§c§lMISSING ARGUMENTS! §cSpecify a player, ban duration and a reason. Valid Usage: /tempban <player> <duration> <reason>");
            return;
        }

        if (args.length == 1) {
            sender.sendMessage("§c§lMISSING ARGUMENTS! §cYou need to add a reason and a ban duration. Valid Usage: /tempban <player> <duration> <reason>");
            return;
        }

        if (args.length == 2) {
            sender.sendMessage("§c§lMISSING ARGUMENTS! §cYou need to add a reason for the ban. Valid Usage: /tempban <player> <duration> <reason>");
            return;
        }
        ...
        if (hybridTarget.isBanned()) {
            sender.sendMessage("§c§lALREADY BANNED! §cThis player already has an active ban on their account!");
            return;
        }
        ...
        if (timeFormatInMillis == 0L)  {
            sender.sendMessage("§c§lERROR! §cSomething went wrong with setting the ban duration. Please try again!");
            return;
        }
        ...
        sender.sendMessage("§aYou successfully temporarily banned " + hybridTarget.getRankManager().getRank().getPrefixSpace() + hybridTarget.getName() + " §afor §b" + timeFormat + " " + durationNormal + "§a, with the reason: §6" + finalReason);
    }
}
